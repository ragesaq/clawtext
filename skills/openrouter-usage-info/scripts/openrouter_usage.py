...
def get_api_key():
    """Resolve OpenRouter API key from environment or OpenClaw auth stores."""
    ...
    # Current auth-profiles layout
    profile_files = sorted(openclaw_dir.glob("agents/*/agent/auth-profiles.json"))
    for profile_file in profile_files:
        try:
            ...
            for profile_name, profile in profiles.items():
                if not str(profile_name).startswith("openrouter"):
                    continue
                token = profile.get("token")
                if token:
                    return token
                token_ref = profile.get("tokenRef") or {}
                if token_ref.get("source") == "file" and token_ref.get("id"):
                    token_path = openclaw_dir / token_ref["id"].lstrip("/")
