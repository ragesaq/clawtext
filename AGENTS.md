...
When a new agent is spawned for a specific task and discovers complex systems, use these onboarding guides:

- **ClawText system onboarding** → `~/.openclaw/workspace/skills/clawtext/AGENT_ONBOARDING.md`
  - Asks user about deduplication practices, cluster rebuild strategy, RAG tuning
  - Takes 10-15 minutes
  - Agent should follow this if ingestion or memory system is part of their scope

**Principle:** New agents should ask smart questions rather than make assumptions about configuration.
...
- **Load when:** Direct chats with owner ID (142843545215041536) - includes Discord DMs and group chats with owner
- **DO NOT load** in shared contexts with non-owner users
...
- **Owner (ID 142843545215041536):** Full read/write to memory files enabled
- **Everyone else:** Read daily notes only, no writing
