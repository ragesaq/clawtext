...
## Current objective
Track how the provider-usage / billing-surface PR should be viewed after the OpenClaw upgrade to 2026.3.11, including whether the core rationale still holds and whether the branch now needs refresh work.
...
1. Rebase or refresh the branch on current upstream main before trying to push the PR over the finish line.
2. Re-run focused tests around models status output and provider usage on top of 2026.3.11 mainline code.
3. Check whether any new status-command changes from upstream should be preserved during conflict resolution instead of blindly replaying the older patch.
4. Continue using the PR as the tracking object for provider-usage JSON parity unless upstream lands overlapping work first.
