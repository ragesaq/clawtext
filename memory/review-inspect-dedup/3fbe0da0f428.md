---
key: 3fbe0da0f428
count: 1
files: 2026-03-04-hmd-cutoff.md
matchedPhrases: ["SUCCESS: Offset applied to device"]
ts: 2026-03-06T18:12:12.338Z
---

# Candidate (count=1)

[ .] -> applying via sharedmemoryipc: pos(x=-.033m, y=-.008m, z=-.001m) [ .] -> success: offset applied to device [ .] settranslationoffsets: role=right, x=-., y=-., z=-. [ .] -> openvr deviceid for right = [ .] -> preserving existing rotation: pitch=., yaw=., roll=. [ .] -> applying via sharedmemoryipc: pos(x=-.033m, y=-.009m, z=-.001m) [ .] -> success: offset applied to device --- _suggested action:_ review this snippet for promotion; annotate priority/category/suggested action if ok.

## Examples

- 2026-03-04-hmd-cutoff.md:409 right matched:["SUCCESS: Offset applied to device"]
  
  [2026-03-04 01:17:35.669] -> Applying via SharedMemoryIPC: pos(x=-0.033m, y=-0.008m, z=-0.001m)

[2026-03-04 01:17:35.676] -> SUCCESS: Offset applied to device 5

[2026-03-04 01:17:35.703] setTranslationOffsets: role=Right, x=-3.27, y=-0.85, z=-0.13

[2026-03-04 01:17:35.703] -> OpenVR deviceId for Right = 5

[2026-03-04 01:17:35.703] -> Preserving existing rotation: pitch=0.673, yaw=0.672, roll=1
