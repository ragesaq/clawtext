---
key: a40a00f5abcc
count: 2
files: 2026-03-04-hmd-cutoff.md
matchedPhrases: ["SUCCESS: Offset applied to device"]
ts: 2026-03-06T18:12:12.309Z
---

# Candidate (count=2)

[ .] -> openvr deviceid for left = [ .] -> preserving existing rotation: pitch=-., yaw=-., roll=-. [ .] -> applying via sharedmemoryipc: pos(x=.022m, y=.008m, z=.036m) [ .] -> success: offset applied to device [ .] settranslationoffsets: role=left, x=-., y=., z=-. [ .] -> openvr deviceid for left = [ .] -> preserving existing rotation: pitch=-., yaw=-., roll=-. --- _suggested action:_ review this snippet for promotion; annotate priority/category/suggested action if ok.

## Examples

- 2026-03-04-hmd-cutoff.md:1470 success matched:["SUCCESS: Offset applied to device"]
  
  [2026-03-04 01:30:12.769] -> OpenVR deviceId for Left = 6

[2026-03-04 01:30:12.769] -> Preserving existing rotation: pitch=-0.000, yaw=-1.206, roll=-4.174

[2026-03-04 01:30:12.769] -> Applying via SharedMemoryIPC: pos(x=0.022m, y=0.008m, z=0.036m)

[2026-03-04 01:30:12.773] -> SUCCESS: Offset applied to device 6

[2026-03-04 01:30:12.799] setTranslationOffsets: role=Left, x=-3.62, y=0.81, z=-2.2

- 2026-03-04-hmd-cutoff.md:1475 success matched:["SUCCESS: Offset applied to device"]
  
  [2026-03-04 01:30:12.799] -> OpenVR deviceId for Left = 6

[2026-03-04 01:30:12.799] -> Preserving existing rotation: pitch=-0.000, yaw=-1.206, roll=-4.174

[2026-03-04 01:30:12.799] -> Applying via SharedMemoryIPC: pos(x=0.022m, y=0.008m, z=0.036m)

[2026-03-04 01:30:12.801] -> SUCCESS: Offset applied to device 6

[2026-03-04 01:30:12.833] setTranslationOffsets: role=Left, x=-3.62, y=0.81, z=-2.2
