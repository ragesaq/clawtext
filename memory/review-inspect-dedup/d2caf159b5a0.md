---
key: d2caf159b5a0
count: 2
files: 2026-03-04-hmd-cutoff.md
matchedPhrases: ["SUCCESS: Offset applied to device"]
ts: 2026-03-06T18:12:12.311Z
---

# Candidate (count=2)

[ .] -> applying via sharedmemoryipc: pos(x=.017m, y=.012m, z=.036m) [ .] -> success: offset applied to device [ .] settranslationoffsets: role=left, x=-., y=., z=-. [ .] -> openvr deviceid for left = [ .] -> preserving existing rotation: pitch=-., yaw=-., roll=-. [ .] -> applying via sharedmemoryipc: pos(x=.017m, y=.012m, z=.036m) [ .] -> success: offset applied to device --- _suggested action:_ review this snippet for promotion; annotate priority/category/suggested action if ok.

## Examples

- 2026-03-04-hmd-cutoff.md:1989 device matched:["SUCCESS: Offset applied to device"]
  
  [2026-03-04 01:30:54.281] -> Applying via SharedMemoryIPC: pos(x=0.017m, y=0.012m, z=0.036m)

[2026-03-04 01:30:54.283] -> SUCCESS: Offset applied to device 6

[2026-03-04 01:30:54.314] setTranslationOffsets: role=Left, x=-3.57, y=1.22, z=-1.71

[2026-03-04 01:30:54.314] -> OpenVR deviceId for Left = 6

[2026-03-04 01:30:54.314] -> Preserving existing rotation: pitch=-0.000, yaw=-1.206, roll=-4.17

- 2026-03-04-hmd-cutoff.md:1994 device matched:["SUCCESS: Offset applied to device"]
  
  [2026-03-04 01:30:54.314] -> Applying via SharedMemoryIPC: pos(x=0.017m, y=0.012m, z=0.036m)

[2026-03-04 01:30:54.318] -> SUCCESS: Offset applied to device 6

[2026-03-04 01:30:54.347] setTranslationOffsets: role=Left, x=-3.57, y=1.22, z=-1.68

[2026-03-04 01:30:54.347] -> OpenVR deviceId for Left = 6

[2026-03-04 01:30:54.347] -> Preserving existing rotation: pitch=-0.000, yaw=-1.206, roll=-4.17
